class Usuario < ActiveRecord::Base
  ...
  validates :nome, presence: true, uniqueness: { case_sensitive: false, message: "já está sendo usado por outra conta" }
  validates :email, presence: true, uniqueness: { case_sensitive: false },
                     format: { with: URI::MailTo::EMAIL_REGEXP, message: "formato inválido" }
  validates :telefone, uniqueness: { message: "já está sendo usado por outra conta" }, allow_blank: true
